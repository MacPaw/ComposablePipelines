...
/// Dataflow between steps is **not** expressed with a typed ``Input``; use ``State`` (or another store)
/// so stages read/write shared context on the runner while this protocol only names each step’s ``Output``.
///
