...
//  elix-toolchain
//
//  Vendored lock-guarded storage — the open package's replacement for AtomicKit's `Synchronized`,
//  so `PipelineDSL` carries no private-SSH dependency. Minimal surface: exactly what
//  `ExecutionContext` uses (wrappedValue, store, projected subscript + write).
//
