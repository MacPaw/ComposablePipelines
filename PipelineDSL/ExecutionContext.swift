...
public struct ExecutionContext: @unchecked Sendable {
    ...
    /// True while lowering a reactive loop body (``While``). Loop-body `@State` writes must NOT be
    /// frozen into reads on re-lowering (the freeze guard that protects one-shot writes from
    /// re-evaluation would otherwise stop the loop from ever advancing).
    @TaskLocal @_spi(Internals) public static var isLoweringLoopBody: Bool = false
