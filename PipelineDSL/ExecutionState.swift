...
public struct Binding<Value: Hashable & Sendable & Codable>: Hashable, Sendable {
    ...
    public struct SetValue<Source: Pipeline>: LeafPipeline where Source.Output == Value {
        ...
        public var pipelineGraph: PipelineGraph {
            ...
            // Captured interpolation/condition reads are sequenced *ahead* of the leaf in both
            // branches. This keeps the surface-task prefix invariant across re-emissions: the
            // engine prefix-skips by ordinal count and asserts the recompiled prefix never shrinks
            // (`PipelineWalker` → `prefixShorterThanRecordedSkipCount`). Dropping these reads when a
            // set freezes to a get would shorten the prefix and silently mis-skip / drop the
            // downstream parallel tasks that follow it.
        ...
        /// During re-lowering (executionEpoch > 0), a slot already committed in the current run
        /// lowers to a read-only get instead of re-running its write. This prevents non-deterministic
        /// model outputs from triggering infinite re-evaluation cycles: each pass would produce a new
        /// value → `previousValue != value` → re-evaluation forever. Only applies to commit writes;
        /// draft writes bypass this check.
        ///
        /// Loop bodies are exempt: a `While` body must re-run its writes on every iteration, so
        /// freezing them would stop the loop after one pass. The engine runs loop bodies inside a
        /// `loopScope` (commits batched, not prefix-skipped), and the loop's own condition plus
        /// `maxReexecutionDepth` bound the iteration count.
        private var freezesToGet: Bool {
            guard writeKind == .commit,
                  ExecutionContext.isEmittingPipelineGraph,
                  !ExecutionContext.isLoweringLoopBody,
