...
actor ExecutionContext {
    ...
    /// Nesting depth of reactive loop scopes (``PipelineExecutionGraph/loopScope(_:)``). While > 0,
    /// commit flushes (and thus `graphProvider` re-execution) are deferred until the iteration body
    /// finishes, and contained tasks bypass prefix-skip and surface-ordinal accounting.
    private var loopScopeDepth = 0
    ...
    func recordStateWrite(
        ...
        case .commit:
            ...
            // Inside a reactive loop body, defer the flush until the whole iteration finishes
            // (``exitLoopScope``) so a mid-body commit doesn't re-execute and tear the iteration apart.
            if parallelDepth == 0 && loopScopeDepth == 0 {
    ...
    /// True while walking a reactive loop body. Contained tasks bypass prefix-skip and do not
    /// consume surface ordinals, so each re-lowering pass re-runs the whole iteration.
    var isInLoopScope: Bool { loopScopeDepth > 0 }
    ...
    func exitLoopScope() {
        loopScopeDepth -= 1
        // Flush the iteration's batched commits once, now that the body is complete — this is what
        // schedules the next re-lowering pass (the next loop iteration, or exit when the body is empty).
        if parallelDepth == 0 && loopScopeDepth == 0 {
            flushCommittedStateUpdates()
        }
    }
    ...
    var observer: (@Sendable (ExecutionEvent) -> Void)? { eventObserver }
    ...
    func executeMemoryQuery(query: String, quality: MemoryQueryQuality) async throws -> [MemoryEntry] {
        guard let memoryProvider else {
            throw ExecutionError.missingMemoryProvider
        }
        return try await memoryProvider.query(query, quality: quality)
    }

    func executeMemoryStore(_ entry: MemoryEntry) async throws {
        guard let memoryProvider else {
            throw ExecutionError.missingMemoryProvider
        }
