...
struct DependencyGraphBuilder: ~Copyable {
    ...
    private mutating func flattenSequence(_ ast: PipelineGraph) -> [UUID] {
        ...
        case .loop:
            // The whole loop body is one reactive node and acts as a gate (barrier): subsequent
            // steps wait for the iteration to complete. The emitter turns it into a `.loopScope`.
            let id = addNode(for: ast, isGate: true)
            return [id]
    ...
    /// barriers, etc.). Slot-only ordering can miss this when the next step does not re-read the
