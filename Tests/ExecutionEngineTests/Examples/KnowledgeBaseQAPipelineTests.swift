...
/// End-to-end execution of ``KnowledgeBaseQAPipeline``: guardrail → retrieve → generate, with
/// the retrieved context actually reaching the model.
final class KnowledgeBaseQAPipelineTests: XCTestCase {
    ...
    func testGuardrailThenRetrieveThenGenerate() async throws {
        let pipeline = KnowledgeBaseQAPipeline(knowledgeBase: knowledgeBase, question: "What is MLX?")
        let (_, events) = try await PipelineRun.runOnce(pipeline, executor: groundingExecutor())

        let guardrailDone = try XCTUnwrap(events.firstCompletedIndex(of: "guardrail"))
        ...
        XCTAssertLessThan(guardrailDone, contextWritten, "Guardrail gates before retrieval.")
