...
final class ContentModerationPipelineDeterminismTests: XCTestCase {
    ...
    /// ``Data.emptyJSON`` (JSON `""`). Guardrail yields JSON `true`, so `severity == "safe"`
    /// is false and the `else` branch runs; the explanation model is the semantic final write.
    private let expectedExplanationBytes = Data.emptyJSON

    /// ``GraphWalker`` returns the last branch's value for a `.parallel` node (indexed by branch
    /// position, so deterministic regardless of completion order — see `GraphWalker.walkCountingNonSkippedTasks`).
    /// When the lowered graph ends in a parallel batch, ``PipelineWalker.run``'s top-level value is that
    /// branch's result — here every mock-model branch yields ``Data.emptyJSON`` — not the pipeline's logical
    /// ``Output``. Assert the explanation **slot** for end-to-end semantics.
