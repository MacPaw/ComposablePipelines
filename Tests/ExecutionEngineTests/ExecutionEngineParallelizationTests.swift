...
final class ExecutionEngineParallelizationTests: XCTestCase {
    ...
    /// Writes an empty `[ContextItem]` array to `id`. Use this instead of `stateSet(_:value:)`
    /// when a ``modelReadSlot`` depends on the slot — the slot must contain valid `[ContextItem]`
    /// JSON or the engine throws ``ExecutionError.contextItemsDecodingFailed``.
    private func contextItemsSet(_ id: UUID) -> PipelineGraph {
        .leaf(.executionStateSet(
            id: id,
            valueTypeName: "[ContextItem]",
            value: .leaf(.just(valueTypeName: "[ContextItem]", jsonUTF8: "[]")),
            debugLabel: nil,
            writeKind: .commit
        ))
    }

    private func modelReadSlot(instructions: String, readingSlot: UUID? = nil) -> PipelineGraph {
    ...
    // MARK: - Gate: predecessor must complete before parallel writes start

    func testGate_parallelGroupStartsAfterGateCompletes() async throws {
        let ast: PipelineGraph = .sequence([
            .group(sequential: true, gate: true, try stringConstant("policy-check")),
        ...
        XCTAssertNotNil(gateDone)
        XCTAssertNotNil(parallelStart)
        XCTAssertLessThan(gateDone!, parallelStart!, "Gate successor must not run in parallel with the gate")
    }
    ...
    func testWriteAfterRead_threeSequentialSteps() async throws {
        let ast: PipelineGraph = .sequence([
            contextItemsSet(slotA),
            modelReadSlot(instructions: "read", readingSlot: slotA),
            contextItemsSet(slotA),
        ])
    ...
    func testConcurrentReadsAfterWrite_twoModelsInParallel() async throws {
        let ast: PipelineGraph = .sequence([
            contextItemsSet(slotA),
            modelReadSlot(instructions: "reader-1", readingSlot: slotA),
            modelReadSlot(instructions: "reader-2", readingSlot: slotA),
        ])
    ...
    func testWriteThenRead_modelRunsAfterSetNotInParallelWithIt() async throws {
        let ast: PipelineGraph = .sequence([
            contextItemsSet(slotA),
            modelReadSlot(instructions: "use", readingSlot: slotA),
        ])
    ...
    func testIndependentWritesThenMerge_parallelThenSequentialModel() async throws {
        let ast: PipelineGraph = .sequence([
            ...
            modelReadSlot(instructions: "Merge", readingSlot: slotA),
        ])
